...
def getArgs():
    ...
    parser.add_argument('-o', '--outputName', default='monoS-2mdm-significances', help='Name of output file (+.png, .pdf, .eps)')
...
def plotLimit(inputFile, args):  # exp, exp_1m, exp_1p, exp_2m, exp_2p):
    ...
    limit = data.pivot(index='ms', columns='mzp', values='exp' if args.expected else 'obs')
    ...
    axes.set_ylabel(r"$m_{s}$ [GeV]",
                    horizontalalignment="right", y=1, labelpad=14, fontsize=20)
    # add text
    model1Text = "$E_{T}^{miss}$ + $s$($VV$): 2MDM"
    model2Text = "$g_{q}$ = 0.25, $g_{\chi}$ = 1, $m_{\\chi}$ = 200\,GeV, $\\theta$ = 0.01"

    drawThesisLabel(left_position, 1.08, axes, energy='13 TeV', lumi=139)

    axes.text(left_position+1., 1.08, model1Text,
              verticalalignment='center', horizontalalignment='right',
              transform=axes.transAxes, fontsize=16)
    axes.text(left_position+1., 1.03, model2Text,
              verticalalignment='center', horizontalalignment='right',
              transform=axes.transAxes, fontsize=16)
...
def main():
    """Create limit plot for the mono-s(VV) analysis with ATLAS style."""
